...
#./Pars_vcf_ANNOVAR_filter_het-aff_hom-unaff_forallsamples_ind.py 1 9535 1
...
#fam7_samp2153339_variants_GATK_step1_norm.vcf
#fam7_samp2153339_variants_GATK_step2_annovar.avinput
#fam7_samp2153339_variants_GATK_step2_annovar.hg19_multianno.vcf

infile11 =[] ;infile22=[]; headr=[]
infile1  = open(AN_ANNOVAR+"samp"+c1+"_"+c2+"_variants_GATK_step1_norm.vcf",'r').readlines() #[200:] #[140:]  #[144:]
for infl in infile1:
	infl = infl.strip()
	if infl[0]!="#":
		infile11.append(infl)
	elif infl[0] =="#":
		headr.append(infl)
headrlen = len(headr)

infile2  = open(AN_ANNOVAR+"samp"+c1+"_"+c2+"_variants_GATK_step2_annovar.hg19_multianno.vcf",'r').readlines() #[223:] #[151:]  #[155:]  
for infl in infile2:
	infl = infl.strip()
	if infl[0]!="#":
		infile22.append(infl)
	
infile3  = open(AN_ANNOVAR+"samp"+c1+"_"+c2+"_variants_GATK_step2_annovar.avinput",'r').readlines()
...
def createCADD(CADD):
	...
	#cmd = CADD > "+TMP+"xa
	#for header 
	cmd1 = "head -"+str(headrlen)+" "+AN_ANNOVAR+"samp"+c1+"_"+c2+"_variants_GATK_step1_norm.vcf > "+TMP+"CADD_"+c2+"_step1_vcfhdr" #-144 "+WDIR+"samp"+c1+"_"+c2+"_variants_step1_norm.vcf
	#---add header to the file
	cmd2 = "cat "+TMP+"CADD_"+c2+"_step1_vcfhdr "+TMP+"xa > "+TMP+"xa_in"
	#---rm ''$TMP''xa$i$i.gz
	cmd3 = "gzip "+TMP+"xa_in"
	#---Run CADD-----
	cmd4 = "/home/varma/softwares/CADD_v1.3/bin/score_anno.sh "+TMP+"xa_in.gz "+TMP+"xa_in.tsv.gz"
	#---unzip-----
...
while (count1 < totlns):
	...
	######---- C = 0/0--or--1/0;0/1 ; D = 1/0;0/1--or--1/1 ; D = 1/0;0/1--or--1/1
	###---If all samples are HET (0/1 or 1/0) use as below or else specify each sample whether true or false...
	#if sum(lst2)==len(bb):
	###--Filter for Homozygous cases (1/1) # order matters recheck!!!
	#if (lst22[0]==True):
	#	False
	###--Filter for Homozygous ref cases (0/0) # order matters recheck!!!
	#if (lst33[1]==True and lst33[2]==True):
	#	False
	###--Filter for Dominant Heterozygous cases (0/1 or 1/0 == True ; 1/1 == False) # order matters recheck!!!
	#if (lst22[0]==True):
        if (lst22[0]==True and lst22[1]==True and lst22[2]==True and lst22[3]==True and lst22[4]==True and lst22[5]==True and lst22[6]==True and lst22[7]==True and lst22[8]==True and lst22[9]==True and lst22[10]==True and lst22[11]==True and lst22[12]==True and lst22[13]==True and lst22[14]==True and lst22[15]==True):
		True
